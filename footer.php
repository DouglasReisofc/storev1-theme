...
<?php if (class_exists('WooCommerce') && storev1_account_enabled()) : ?><div><h2>Minha conta</h2><a href="<?php echo esc_url(wc_get_page_permalink('myaccount')); ?>">Acessar minha conta</a><a href="<?php echo esc_url(wc_get_account_endpoint_url('orders')); ?>">Histórico de compras</a><?php if(get_privacy_policy_url()) : ?><a href="<?php echo esc_url(get_privacy_policy_url()); ?>">Privacidade</a><?php endif; ?></div><?php elseif (get_privacy_policy_url()) : ?><div><h2>Informações</h2><a href="<?php echo esc_url(get_privacy_policy_url()); ?>">Privacidade</a></div><?php endif; ?>
