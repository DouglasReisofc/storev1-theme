...
function storev1_render_storefront_modals() {
    ...
    $register_url = function_exists('storev1_account_url') ? storev1_account_url('register') : wc_get_page_permalink('myaccount');
    $register_url = add_query_arg('storev1_return_checkout', '1', $register_url);
    ...
    echo '<div class="sv1-checkout-login-copy"><span class="sv1-modal-eyebrow">Compra segura</span><h2 id="sv1-checkout-login-title">Entre para continuar sua compra</h2><p>Encontramos um cadastro com este e-mail. Entre agora e voltaremos automaticamente ao checkout, sem perder seu carrinho.</p>';
    echo '<div class="sv1-checkout-login-banners"><div class="sv1-checkout-login-banner"><img src="' . esc_url(get_template_directory_uri() . '/assets/account-login-illustration.webp') . '" alt="Ilustração de login" loading="lazy"><strong>Já sou cliente</strong><span>Acesse seus pedidos e continue de onde parou.</span></div><div class="sv1-checkout-login-banner"><img src="' . esc_url(get_template_directory_uri() . '/assets/account-register-illustration.webp') . '" alt="Ilustração de cadastro" loading="lazy"><strong>Ainda não tenho conta</strong><a href="' . esc_url($register_url) . '" target="_blank" rel="noopener">Criar cadastro</a></div></div></div>';
    echo '<form class="sv1-checkout-login-form" data-sv1-checkout-login-form novalidate><label for="sv1-checkout-login-email">E-mail</label><input id="sv1-checkout-login-email" type="email" data-sv1-login-email autocomplete="email" required><label for="sv1-checkout-login-password">Senha</label><input id="sv1-checkout-login-password" type="password" data-sv1-login-password autocomplete="current-password" required><label class="sv1-checkout-login-remember"><input type="checkbox" data-sv1-login-remember> Lembrar acesso neste dispositivo</label><button type="submit" class="sv1-checkout-login-submit">Entrar e continuar a compra</button><button type="button" class="sv1-checkout-login-secondary" data-sv1-login-close>Continuar como visitante</button><a class="sv1-checkout-login-forgot" href="' . esc_url(wp_lostpassword_url()) . '" target="_blank" rel="noopener">Esqueci minha senha</a><p class="sv1-checkout-login-feedback" data-sv1-login-feedback role="status" aria-live="polite"></p></form>';
    echo '</div></div>';
    }
    echo '</dialog>';
}
