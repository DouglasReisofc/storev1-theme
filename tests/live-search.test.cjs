...
function setup(){
  const node=()=>({events:{},dataset:{},attributes:{},hidden:false,value:'',
    ...
    addEventListener(k,fn){this.events[k]=fn;},focus(){},contains(){return false;}});
  const input=node(),results=node(),status=node(),close=node(),banner=node(),grid=node();
  grid.classList={toggle(k,v){grid.searchHidden=v;}};
  const toolbar={prepend(n){this.toggle=n;},querySelector(){return grid;},insertBefore(n,before){this.panel=n;this.before=before;}};
  const count={parentElement:toolbar,remove(){this.removed=true;}};
  const document={
    querySelector(s){return s==='.woocommerce-result-count'?count:banner;},
