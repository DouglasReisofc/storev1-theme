(() => {
  const boot = () => {
    document.querySelectorAll('form.variations_form').forEach(form => {
      const select = form.querySelector('select[name="attribute_pa_validade"]');
      ...
      [...select.options].filter(option => option.value).forEach(option => {
        const variation = variations.find(item => item.attributes && item.attributes.attribute_pa_validade === option.value);
        if (!variation) return;
        ...
        const image = document.createElement('img'); image.className = 'sv1-offer-image'; image.alt = ''; image.loading = 'lazy'; image.src = variation.storezap_offer_image || variation.image?.src || ''; if (!image.src) image.hidden = true;
        const details = document.createElement('span'); details.className = 'sv1-offer-details'; const title = document.createElement('strong'); title.className = 'sv1-offer-title'; title.textContent = variation.storezap_offer_title || option.textContent.trim(); const hint = document.createElement('small'); hint.textContent = 'Oferta disponível'; details.append(title, hint);
